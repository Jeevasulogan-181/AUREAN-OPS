...
<div class="fixed inset-0 z-10 flex items-center justify-center bg-slate-900/40 px-4">
	<div class="max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-xl bg-white p-6 shadow-lg">
		<div class="mb-4 flex items-center justify-between">
			<h2 class="text-lg font-semibold text-slate-900">New channel</h2>
		...
		<form
			...
			use:enhance={() => {
				...
				return async ({ result, update }) => {
					...
					// Success is always a redirect into the new channel — the
					// [[channelId]] page reuses this same component instance
					// across that navigation, so the modal must be told to
					// close explicitly or it stays stacked on top.
					await update();
					onclose();
				};
			}}
		>
			<div>
				<label for="name" class="mb-1 block text-sm font-medium text-slate-700">Channel name</label>
				<input
					id="name"
					name="name"
					type="text"
				...
				<div class="max-h-48 space-y-1 overflow-y-auto rounded-lg border border-slate-200 p-2">
					{#each staffList as staffMember (staffMember.id)}
						<label class="flex items-center gap-2 rounded px-1.5 py-1 text-sm hover:bg-slate-50">
							<input type="checkbox" name="memberIds" value={staffMember.id} />
